...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Seeding & Auto-Dispatching Webhooks on First Run
    const alreadySeeded = localStorage.getItem('revitalize_seeded_webhooks');
    if (!alreadySeeded) {
        // 1. Seed sample lead
        const sampleLead = {
            id: 'lead-sample-123',
            ...
            stage: 'rehab',
            workExplanations: { paint: 'Wants nice white interior paint.', kitchen: 'Update cabinets to shaker style.' }
        };
        
        if (!leads.some(l => l.id === sampleLead.id)) {
            leads.push(sampleLead);
            saveLeadsToStorage();
        }
        ...
        if (!adCampaigns.some(a => a.id === adFb.id)) {
            adCampaigns.push(adFb, adIg);
            saveAdCampaignsToStorage();
        }

        // 3. Dispatch webhooks directly to Make.com so the data structure is captured immediately
        const webhookUrl = 'https://hook.us2.make.com/pr1l0jvjtuto0qikljvjkrr82jtttmz1';
        ...
        fetch(webhookUrl, {
            ...
            body: JSON.stringify({
                ...
                campaignId: adFb.id,
                property: adFb.property,
                channel: adFb.channel,
                budget: adFb.budget,
        ...
        fetch(webhookUrl, {
            ...
            body: JSON.stringify({
                event: 'ad_campaign_launched',
                campaignId: adIg.id,
                property: adIg.property,
                channel: adIg.channel,
                budget: adIg.budget,
                headline: '📸 Invest in our Skills to create Capital at 123 Main St! Get beautiful renovations with zero out-of-pocket fees.',
                timestamp: new Date().toISOString()
            })
        });
        ...
        const sampleLog = {
            ...
            recipient: 'sean@example.com',
            subject: 'Invest in our Skills to create Capital at 123 Main St.',
            body: 'Outbound campaign drip initiated.',
            timestamp: new Date().toISOString()
        };
        emailLogs.unshift(sampleLog);
        localStorage.setItem('revitalize_email_logs', JSON.stringify(emailLogs));

        localStorage.setItem('revitalize_seeded_webhooks', 'true');
